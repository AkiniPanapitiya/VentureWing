...
export const SupplierCard: React.FC<SupplierProps> = ({
  id,
  name,
  country,
  location,
  ...
  return (
    <div
      className={`bg-white rounded-2xl border p-6 flex flex-col justify-between transition-all duration-200 hover:shadow-lg relative group ${
        recommended
      ...
      <div>
        ...
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/80 mb-5">
          <div className="flex justify-between items-baseline mb-2">
            ...
            <span className="text-lg font-black text-slate-900">${fobPrice.toFixed(2)} USD</span>
          </div>
          <div className="flex justify-between items-baseline pt-2 border-t border-slate-200/60">
            ...
            <div className="text-right">
              <span className="text-sm font-extrabold text-indigo-600">${landedCostUsd.toFixed(2)} USD</span>
              <p className="text-[11px] font-semibold text-slate-500">
                (~LKR {landedCostLkr.toLocaleString()})
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs mb-6">
          <div className="bg-white p-2.5 rounded-lg border border-slate-100">
            ...
            <span className="font-bold text-slate-800 mt-0.5 block">{leadTime}</span>
          </div>
          <div className="bg-white p-2.5 rounded-lg border border-slate-100">
            <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block">
              Capacity
            </span>
            <span className="font-bold text-slate-800 mt-0.5 block">{capacity}</span>
          </div>
        </div>
      </div>
